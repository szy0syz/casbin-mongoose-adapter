...
module.exports = class Adapter {
  ...
  async init() {
    ...
    mongoose.connection.on('open', () => {
      console.log('Connected to MongoDB ', this.dbUri)
    })
  ...
  async removePolicy(sec, ptype, rules) {
    ...
    console.log('~~~CasbinRule.remove({ where })~~', where)
    console.log('^^^^^^^^^^^^^^^^')
    await CasbinRule.remove({ where })
  }
  ...
  async removeFilteredPolicy(sec, ptype, fieldIndex, ...fieldValues) {
    ...
    console.log('~~removeFilteredPolicy~~~~~CasbinRule.remove({ where })', where);
    await CasbinRule.remove({ where })
  }
}
